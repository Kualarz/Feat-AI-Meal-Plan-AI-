...
export default function DislikesPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/preferences', { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(d => {
        if (d?.dislikes) setSelected(d.dislikes.split(',').map((s: string) => s.trim()).filter(Boolean));
      })
      .catch(() => {});
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      await fetch('/api/preferences', {
        ...
        body: JSON.stringify({ dislikes: selected.join(',') }),
      });
      setSaved(true);
      setIsDirty(false);
  ...
  const customItems = selected.filter(s => !COMMON_DISLIKES.find(d => d.name === s));
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="sticky top-16 z-10 bg-background/95 backdrop-blur-md border-b border-border/40">
        ...
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-base font-display text-foreground tracking-tight flex-1">Food Dislikes</h1>
          ...
          {customItems.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {customItems.map(item => (
                <span key={item} className="flex items-center gap-1 px-2.5 py-1 bg-rose-500/10 text-rose-500 rounded-pill text-xs font-display">
                  {item}
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
        <p className="text-xs text-muted-foreground font-body leading-relaxed">
          Tap any ingredient to toggle it. Press Save when done.
        </p>

        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          {COMMON_DISLIKES.map((d, i) => {
            const isSelected = selected.includes(d.name);
            return (
              <button
                key={d.name}
                onClick={() => toggle(d.name)}
                className={`w-full flex items-center gap-3.5 px-4 py-4 text-left transition-colors ${i < COMMON_DISLIKES.length - 1 ? 'border-b border-border/50' : ''} ${isSelected ? 'bg-rose-500/5' : 'hover:bg-muted/30 active:bg-muted/50'}`}
              >
                ...
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? 'bg-rose-500 border-rose-500' : 'border-border'}`}>
